...
class CitySeeder extends Seeder
{
    ...
    {
        // Your communes data (paste as many as you want / have)
        $communes = [
    [
        "id"=> 22,
 ...
 $count = 0;
        ...
        foreach ($communes as $commune) {
            // Important=> your WilayaSeeder uses code '1' (without leading zero)
            // so we normalize here
            $wilayaCode = ltrim($commune['wilaya_code'], '0');

            $wilaya = Wilaya::where('code', $wilayaCode)->first();

            if (!$wilaya) {
                $this->command->warn(
                    "Wilaya code {$commune['wilaya_code']} not found → skipping {$commune['commune_name_ascii']}"
                );
                $skipped++;
                continue;
            }
            ...
            City::updateOrCreate(
                [
                    'name_ar'   => trim($commune['commune_name']),
                    'wilaya_id' => $wilaya->id,
                ],
                [
                    'name_en'   => trim($commune['commune_name_ascii']),
                ]
            );
        ...
        if ($skipped > 0) {
            $this->command->warn("$skipped communes were skipped (wilaya missing)");
        }
    }
}